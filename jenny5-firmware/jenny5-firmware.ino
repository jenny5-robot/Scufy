...
byte motors_enable_pins[] = {4, 7, 10, 13};
byte potentiometers_pins[] = {0, 1, 2, 3};
...
char firmware_version[] = "2015.11.19.2";// year.month.day.version
      ...
      for (int i = 0; i < buffer_length; i++)
        if (current_buffer[i] >= 'A' && current_buffer[i] <= 'Z' || current_buffer[i] >= 'a' && current_buffer[i] <= 'z')
        {
          ...
          if (j < buffer_length)
          {
              ...
              else if (current_buffer[i] == 'D' || current_buffer[i] == 'd') // disables motor
              {
                  int motor_index;
                  sscanf(tmp_str, "%d", &motor_index);
                  motors_control.disable_motor(motor_index);
              }
              else if (current_buffer[i] == 'L' || current_buffer[i] == 'l') // locks motor
              {
                    int motor_index;
                    sscanf(tmp_str, "%d", &motor_index);
                    motors_control.lock_motor(motor_index);
              }
              else if (current_buffer[i] == 'S' || current_buffer[i] == 's') // motor speed
              {
              ...
              else if (current_buffer[i] == 'A' || current_buffer[i] == 'a') // motor acceleration
              {
              ...
              else if (current_buffer[i] == 'U' || current_buffer[i] == 'u') // ultrasonic
              {
                    int sensor_index;
                    sscanf(tmp_str, "%d", &sensor_index);
                    Serial.print(ultrasonic.Ranging());
              }
              else if (current_buffer[i] == 'P' || current_buffer[i] == 'p') // potentiometer
          ...
          {// the string is not completed ... so I must wait more...
            break; // for i
          }
        }
    }
